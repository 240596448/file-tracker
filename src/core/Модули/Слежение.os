...
Функция СвойстваФайла(Файл)
	сткСвойства = СвойстваПоУмолчанию();
	Попытка
	...
	Исключение
		...
		Лог.Ошибка("Ошибка чтения свойств файла
					|%1", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
	Возврат сткСвойства;
КонецФункции

Функция СвойстваПоУмолчанию()
	
	стк = Новый Структура;
	
	стк.Вставить("ПолноеИмя", "");
	стк.Вставить("Имя", "");
	стк.Вставить("ОтносительныйПуть", "");
	стк.Вставить("ЭтоКаталог", Ложь);
	стк.Вставить("Размер", 0);
	стк.Вставить("ДатаИзменения", '00010101');
	стк.Вставить("Статус", Неопределено);
	стк.Вставить("ДатаКонтроля", '00010101');
...
Процедура Инициализировать()
	...
	Для Каждого КЗ Из СвойстваПоУмолчанию() Цикл
		ТаблицаКонтроля.Колонки.Добавить(КЗ.Ключ);
	КонецЦикла;
