...
Процедура ПослеЧтенияПараметров() Экспорт
	...
	Если Включен() Тогда
		...
		ЛогЛокально = Логирование.ПолучитьЛог(ПараметрыПриложения.ИмяЛога()+".arc");
		ИмяФайлаЛога = "file-tracker-" + Формат(ТекущаяДата(), "ДФ=yyyy-MM-dd_HH-mm-ss") + ".log";
		Аппендер = Новый ВыводЛогаВФайл();
		Аппендер.ОткрытьФайл(ОбъединитьПути(Параметры.Путь, ИмяФайлаЛога));
		ЛогЛокально.ДобавитьСпособВывода(Аппендер);
...
Процедура ВыполнитьДействие(мФайлыКОбработке) Экспорт
	...
	Для каждого СтрокаТаблицыКонтроля Из мФайлыКОбработке Цикл
		...
		ЛогЛокально.Информация("%1: %2 (размер:%3) - файл архива: %4", 
						СтрокаТаблицыКонтроля.Статус, 
						СтрокаТаблицыКонтроля.Имя, 
						СтрокаТаблицыКонтроля.Размер, 
						ИмяАрхива);
	КонецЦикла;
